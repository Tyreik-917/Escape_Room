...
class Color(Item):
    def interact(self):
        ...
        if has_red_light and has_blue_light and has_green_light and has_gray_light:
            ...
            return
        if won:
            self.show_message("You won the memory game! You got a KEY.")
            self.is_finished = True
            self.interactable = False
            self.reinteractable = False
            self.is_active = False
        else:
            self.show_message("You failed the memory game. Try again later.")
...
class SmokeDetector(Item):
    def interact(self):
        ...
        if has_ladder == False:
            ...
            pygame.mixer.Sound("smoke_detector_beep.mp3").play()
